...
def generateDataWindowForce(window_size, ctrl_horizon):
    ...
    out_train = np.array([]).reshape(0, 18 * ctrl_horizon)
    for i in range(walking_ranges[0][0],walking_ranges[0][1]):
        temp_ctrl = np.array([]).reshape(1, 0)
        temp_output = np.array([]).reshape(1, 0)
        for j in range(ctrl_horizon):
    ...
    out_val = np.array([]).reshape(0, 18 * ctrl_horizon)
    for i in range(walking_ranges[1][0],walking_ranges[1][1]):
        temp_ctrl = np.array([]).reshape(1, 0)
        temp_output = np.array([]).reshape(1, 0)
        for j in range(ctrl_horizon):
    ...
    out_test = np.array([]).reshape(0, 18 * ctrl_horizon)
    for i in range(walking_ranges[2][0],walking_ranges[2][1]):
        temp_ctrl = np.array([]).reshape(1, 0)
        temp_output = np.array([]).reshape(1, 0)
        for j in range(ctrl_horizon):
...
def buildModelWindow(
    ...
    last_pred_layer = build_network_block(
        regularizer_rate,
        pred_layer_size,
        # layers.Concatenate()([last_ctrl_layer, input_layer[:, -5:-1]]), # we only input the state of last step instead of the last 10 steps
        last_ctrl_layer,
    ...
    tf_callback = [
        ...
        keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", factor=0.1, patience=7, min_lr=0.00001,
        ),
...
def hand_labeling_data(train_out, test_out, control_horizon, state_bounds, gap = 0.2):
    for key, value in state_bounds.items():
        for i in range(control_horizon):
            train_out[:, control_horizon + 4*i+key] = np.clip(
                train_out[:, control_horizon + 4*i+key], value[0]+gap, value[1]-gap
                )
            test_out[:, control_horizon + 4*i+key] = np.clip(
                test_out[:, control_horizon + 4*i+key], value[0]+gap, value[1]-gap
                )
    return train_out, test_out


def merge_models(full_model, ctrl_model, pred_model):
    pred_num_layers = len(pred_model.layers)
    ctrl_num_layers = len(ctrl_model.layers)
    layer_id = 0
    for i in range(ctrl_num_layers):
        if len(ctrl_model.layers[i].get_weights()) != 0:
            while len(full_model.layers[layer_id].get_weights()) == 0:
                layer_id += 1
            full_model.layers[layer_id].set_weights(
                ctrl_model.layers[i].get_weights()
            )
            layer_id += 1
    for i in range(pred_num_layers):
        if len(pred_model.layers[i].get_weights()) != 0:
            while len(full_model.layers[layer_id].get_weights()) == 0:
                layer_id += 1
            full_model.layers[layer_id].set_weights(
                pred_model.layers[i].get_weights()
            )
            layer_id += 1
    return full_model


def buildModelSeparate(inp_size, arch, block_name="ctrl"):
    regularizer_rate = 0.001

    input_layer = tf.keras.Input(shape=(inp_size))
    last_layer = build_network_block(
        regularizer_rate,
        arch,
        input_layer,
        block_name,
...
if __name__ == "__main__":
    ...
    if not os.path.exists("models"):
        os.mkdir("models")
    if not os.path.exists("pred_models"):
        os.mkdir("pred_models")
    if not os.path.exists("ctrl_models"):
        os.mkdir("ctrl_models")

    # parameters
    now = datetime.now()
    now_str = f"_{now.month}_{now.day}_{now.year}_{now.hour}_{now.minute}_{now.second}"
    pred_trained_time_str = None  # specify the time the model was trained
    ctrl_trained_time_str = None

    pred_load = False
    ctrl_load = False
    num_states_obs = 2        # tibia angle and angular velocity
    num_states_non_obs = 16   # 16 force sensors
    num_hidden_nodes = 128
    batch_size = 25
    epochs = 100
    control_horizon = 10
    pred_inp_order = ["control", "state"]
    ...
    if pred_load:
        pred_model_orig = keras.models.load_model(
            os.path.dirname(os.path.realpath(__file__))
            + f"/pred_models/model_{control_horizon}_{num_hidden_nodes}{pred_trained_time_str}"
        )
    else:
        # build predictor block
        pred_model_orig, callback, architecture = buildModelSeparate(
            inp_size=control_horizon,
            arch=pred_layer_size,
            block_name="pred",
        )
        if pred_inp_order[0] == "control":
            # train predictor model, input = ["control", "state"]
            train_pred_inp = np.concatenate(
                (
                    train_out[:, 0:control_horizon],
                    train_obs[:, -5:-1],
                ), axis=1
            )
            test_pred_inp = np.concatenate(
                (
                    test_out[:, 0:control_horizon],
                    test_obs[:, -5:-1],
                ), axis=1
            )
        else:
            # train predictor model, input = ["state", "control"]
            train_pred_inp = np.concatenate(
                (
                    train_obs[:, -5:-1],
                    train_out[:, 0:control_horizon],
        ...
        pred_model_orig.fit(
            train_out[:, 0:control_horizon],
            train_out[:, control_horizon:],
            validation_data=(test_out[:, 0:control_horizon], test_out[:, control_horizon:]),
        ...
        keras.models.save_model(
            pred_model_orig,
            os.path.dirname(os.path.realpath(__file__))
            + f"/pred_models/model_{control_horizon}_{num_hidden_nodes}{now_str}",
            overwrite=True,
    ...
    if ctrl_load:
        ctrl_model_orig = keras.models.load_model(
            os.path.dirname(os.path.realpath(__file__))
            + f"/ctrl_models/model_{control_horizon}_{num_hidden_nodes}{ctrl_trained_time_str}"
        )
    else:
        # build controller block
        ctrl_model_orig, callback, architecture = buildModelSeparate(
            inp_size=train_obs.shape[1],
            arch=ctrl_layer_size,
            block_name="ctrl",
        )
        ...
        ctrl_model_orig.fit(
            train_obs,
            train_out[:, 0:control_horizon],
            validation_data=(test_obs, test_out[:, 0:control_horizon]),
            batch_size=batch_size,
            epochs=epochs,
            use_multiprocessing=True,
            verbose=1,
        ...
        keras.models.save_model(
            ctrl_model_orig,
            os.path.dirname(os.path.realpath(__file__))
            + f"/ctrl_models/model_{control_horizon}_{num_hidden_nodes}{now_str}",
            overwrite=True,
            include_optimizer=False,
            save_format=None,
            signatures=None,
    ...
    model_orig, _, _ = buildModelWindow(
        train_obs.shape, ctrl_layer_size, pred_layer_size
    )
    # merge models
    model_orig = merge_models(model_orig, ctrl_model_orig, pred_model_orig)
    ...
    #     validation_data=(test_obs, [test_out[:, control_horizon:], test_out[:, 0:control_horizon]]),
    #     batch_size=batch_size,
    #     epochs=50,
    #     use_multiprocessing=True,
    #     verbose=1,
    ...
    # print(f"weight norm layer {1} is {np.linalg.norm(model_orig.layers[1].get_weights()[0])}")
    # print(f"weight norm layer {2} is {np.linalg.norm(model_orig.layers[2].get_weights()[0])}")
    # print(f"weight norm layer {3} is {np.linalg.norm(model_orig.layers[3].get_weights()[0])}")
    # print(f"weight norm layer {6} is {np.linalg.norm(model_orig.layers[6].get_weights()[0])}")
    # print(f"weight norm layer {7} is {np.linalg.norm(model_orig.layers[7].get_weights()[0])}")
    # ctrl_model_orig.layers[1].trainable = False
    # ctrl_model_orig.layers[3].trainable = False
    model_orig.layers[4].trainable = False
    model_orig.layers[5].trainable = False
    ...
    def loss3(y_true, y_pred):
        loss = 0
        const_state = 3
        for i in range(4*control_horizon):
            if i % 4 == const_state:
                loss += 1*tf.keras.losses.MSE(y_true[:, i], y_pred[:, i])
            else:
                loss += tf.keras.losses.MSE(y_true[:, i], y_pred[:, i])
        return loss
    ...
    train_out, test_out = hand_labeling_data(
        train_out, test_out, control_horizon, {3: [-1.4, 2]}, 0.5)
    model_orig.fit(
        train_obs,
        # [train_out[:, control_horizon:], train_out[:, 0:control_horizon]],
        train_out[:, 0:control_horizon],
    ...
    print("saved: model")
    plotTestData(model_orig, test_obs, test_out, control_horizon)
