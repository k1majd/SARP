...
def buildModelWindow(
    ...
    last_pred_layer = build_network_block(
        regularizer_rate,
        pred_layer_size,
        layers.Concatenate()([last_ctrl_layer, input_layer[:, -3:-1]]),  # we only input the state of last step instead of the last 10 steps
...
if __name__ == "__main__":
    ...
    orig_load = True
    trained_time_str = "_2_16_2023_10_7_31"
    # Train window model
    # (
    #     train_obs,
    #     train_out,
    ...
    def loss3(y_true, y_pred):
        ...
        # loss += 10*tf.keras.losses.MSE(y_true[:,1], y_pred[:,1])
        # loss += tf.keras.losses.MSE(y_true[:,2], y_pred[:,2])
        # loss += tf.keras.losses.MSE(y_true[:,3], y_pred[:,3])
        # loss += tf.keras.losses.MSE(y_true[:,4], y_pred[:,4])
        # loss += 10*tf.keras.losses.MSE(y_true[:,5], y_pred[:,5])
        # loss += tf.keras.losses.MSE(y_true[:,6], y_pred[:,6])
        # loss += tf.keras.losses.MSE(y_true[:,7], y_pred[:,7])
        # loss += tf.keras.losses.MSE(y_true[:,8], y_pred[:,8])
        # loss += 10*tf.keras.losses.MSE(y_true[:,9], y_pred[:,9])
        # loss += tf.keras.losses.MSE(y_true[:,10], y_pred[:,10])
        # loss += tf.keras.losses.MSE(y_true[:,11], y_pred[:,11])
        # loss += tf.keras.losses.MSE(y_true[:,12], y_pred[:,12])
        # loss += 10*tf.keras.losses.MSE(y_true[:,13], y_pred[:,13])
        # loss += tf.keras.losses.MSE(y_true[:,14], y_pred[:,14])
        # loss += tf.keras.losses.MSE(y_true[:,15], y_pred[:,15])
        # loss += tf.keras.losses.MSE(y_true[:,16], y_pred[:,16])
        # loss += 10*tf.keras.losses.MSE(y_true[:,17], y_pred[:,17])
        # loss += tf.keras.losses.MSE(y_true[:,18], y_pred[:,18])
        # loss += tf.keras.losses.MSE(y_true[:,19], y_pred[:,19])
        return loss
    ...
    train_out, val_out = hand_labeling_data(
        ...
        val_out,
        control_horizon,
        {2: [-1.4, 1], 3: [-1.4, 1], 4: [-1.4, 1], 5: [-1.4, 1]},
        0.3,
