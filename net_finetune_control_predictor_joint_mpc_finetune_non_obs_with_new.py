...
def buildModelWindow(
    ...
    last_pred_layer = build_network_block(
        regularizer_rate,
        pred_layer_size,
        layers.Concatenate()([last_ctrl_layer, input_layer[:, -2:]]),  # we only input the state of last step instead of the last 10 steps
...
if __name__ == "__main__":
    ...
    orig_load = True
    # trained_time_str = "_2_24_2023_22_11_1"
    trained_time_str = "_2_25_2023_0_58_21"
    # Train window model
    # (
    #     train_obs,
    #     train_out,
    ...
    (
        ...
        a_p_test,
        s_p_test,
    ) = generateDataWindowForce3(10, control_horizon)
    train_obs3 = s_h_train
    train_out3 = np.concatenate((a_p_train, s_p_train), axis=1)
    val_obs3 = s_h_val
    val_out3 = np.concatenate((a_p_val, s_p_val), axis=1)
    test_obs3 = s_h_test
    test_out3 = np.concatenate((a_p_test, s_p_test), axis=1)
    ...
    plotTestDataSubForce(ctrl_model_orig, test_obs3, test_out3, control_horizon)
    ...
    ctrl_model_orig.layers[5].trainable = True
    ctrl_model_orig.layers[6].trainable = True
    ...
    ctrl_model_orig.fit(
        val_obs3,
        [val_out3[:, control_horizon:], val_out3[:, 0:control_horizon]],
        validation_data=(
            train_obs3, [train_out3[:, control_horizon:], train_out3[:, 0:control_horizon]]
            ),
        batch_size=5,
        epochs=2,
        use_multiprocessing=True,
    ...
    plotTestDataSubForce(ctrl_model_orig, train_obs3, train_out3, control_horizon)
    plotTestDataSubForce(ctrl_model_orig, test_obs3, test_out3, control_horizon)
    plotTestDataSubForce(ctrl_model_orig, val_obs3, val_out3, control_horizon)
