...
def generateDataWindowForce(window_size, ctrl_horizon):
    ...
    obs_train = np.array([]).reshape(0, 3 * window_size)
    ...
    for i in range(walking_ranges[0][0], walking_ranges[0][1]):
        ...
        for j in range(ctrl_horizon):
            ...
            temp_output = np.concatenate(
                (
                    temp_output,
                    observation[i + window_size + j, :-1].reshape(1, -1),
                ),
                axis=1,
            )
            temp_output = np.concatenate(
    ...
    obs_val = np.array([]).reshape(0, 3 * window_size)
    ...
    # create test dataset
    obs_test = np.array([]).reshape(0, 3 * window_size)
...
def generateDataWindowForce2(window_size, ctrl_horizon):
    ...
    # create training dataset
    obs_train = np.array([]).reshape(0, 8 * window_size)
    for i in range(walking_ranges[0][0],walking_ranges[0][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
    ...
    # create validation dataset
    obs_val = np.array([]).reshape(0, 8 * window_size)
    for i in range(walking_ranges[1][0], walking_ranges[1][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
    ...
    # create test dataset
    obs_test = np.array([]).reshape(0, 8 * window_size)
    for i in range(walking_ranges[2][0], walking_ranges[2][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
...
def buildModelWindow(
    ...
    last_pred_layer = build_network_block(
        regularizer_rate,
        pred_layer_size,
        layers.Concatenate()([last_ctrl_layer, input_layer[:, -9:-1]]),  # we only input the state of last step instead of the last 10 steps
...
def plotTestDataSubForce(model, test_obs, test_ctrls, control_horizon):
    ...
    # control diff calc
    control_diff_ref = test_ctrls[:, 0] - test_obs[:, -1]
    control_diff_pred = pred_ctrls[1][:, 0] - test_obs[:, -1]
    ax4.plot(
        control_diff_ref.flatten()[:x_lim],
        # label=f"test control {i+1}",
        color=colors[0],
        linestyle="dashed",
    )
    ax4.plot(
        control_diff_pred.flatten()[:x_lim],
        label=f"Control Diff {1}",
        color=colors[0],
    )
...
def plotTestDataSubForce(model, test_obs, test_ctrls, control_horizon):
    ...
    control_diff_ref = test_ctrls[:, 0] - test_obs[:, -1]
    control_diff_pred = pred_ctrls[1][:, 0] - test_obs[:, -1]
    state_diff_ref4 = test_ctrls[:, control_horizon+4] - test_obs[:, -5]
    state_diff_pred4 = pred_ctrls[0][:, 4] - test_obs[:, -5]
    state_diff_ref5 = test_ctrls[:, control_horizon+5] - test_obs[:, -4]
    state_diff_pred5 = pred_ctrls[0][:, 5] - test_obs[:, -4]
    state_diff_ref6 = test_ctrls[:, control_horizon+6] - test_obs[:, -3]
    state_diff_pred6 = pred_ctrls[0][:, 6] - test_obs[:, -3]
    state_diff_ref7 = test_ctrls[:, control_horizon+7] - test_obs[:, -2]
    state_diff_pred7 = pred_ctrls[0][:, 7] - test_obs[:, -2]
    ax4.plot(
        control_diff_ref.flatten()[:x_lim],
        # label=f"test control {i+1}",
        color=colors[0],
    ...
    ax5.plot(
        state_diff_ref4.flatten()[:x_lim],
        color=colors[4],
        linestyle="dashed",
    )
    ax5.plot(
        state_diff_pred4.flatten()[:x_lim],
        label=f"State Diff {4}",
        color=colors[4],
    )
    ax5.plot(
        state_diff_ref5.flatten()[:x_lim],
        color=colors[5],
        linestyle="dashed",
    )
    ax5.plot(
        state_diff_pred5.flatten()[:x_lim],
        label=f"State Diff {5}",
        color=colors[5],
    )
    ax5.plot(
        state_diff_ref6.flatten()[:x_lim],
        color=colors[6],
        linestyle="dashed",
    )
    ax5.plot(
        state_diff_pred6.flatten()[:x_lim],
        label=f"State Diff {6}",
        color=colors[6],
    )
    ax5.plot(
        state_diff_ref7.flatten()[:x_lim],
        color=colors[7],
        linestyle="dashed",
    )
    ax5.plot(
        state_diff_pred7.flatten()[:x_lim],
        label=f"State Diff {7}",
        color=colors[7],
    )
...
def hand_labeling_data2(train_out, test_out, control_horizon, state_bounds, gap=0.2, num_states=6):
...
def hand_labeling_data(train_obs, train_out, test_obs, test_out, control_horizon, control_bound, state_bounds, gap_state=0.2, gap_ctrl=1., num_states=6):
    num_states = (train_out.shape[1] - control_horizon) // control_horizon
    # apply state rate bound on train
    for key, value in state_bounds.items():
        for i in range(1):
            if i == 0:
                diff = train_out[:, control_horizon + num_states*i+key] - train_obs[:, (key-num_states-1)]
            else:
                diff = train_out[:, control_horizon + num_states*i+key] - train_out[:, control_horizon + num_states*(i-1)+key]
            diff = np.clip(
                diff,
                value[0] + gap_state,
                value[1] - gap_state,
            ...
            if i == 0:
                train_out[:, control_horizon + num_states*i+key] = train_obs[:, (key-num_states-1)] + diff
            else:
                train_out[:, control_horizon + num_states*i+key] = train_out[:, control_horizon + num_states*(i-1)+key] + diff

    # apply state rate bound on test
    for key, value in state_bounds.items():
        for i in range(1):
            if i == 0:
                diff = test_out[:, control_horizon + num_states*i+key] - test_obs[:, (key-num_states-1)]
            else:
                diff = test_out[:, control_horizon + num_states*i+key] - test_out[:, control_horizon + num_states*(i-1)+key]
            diff = np.clip(
                diff,
                value[0] + gap_state,
                value[1] - gap_state,
            ...
            if i == 0:
                test_out[:, control_horizon + num_states*i+key] = test_obs[:, (key-num_states-1)] + diff
            else:
                test_out[:, control_horizon + num_states*i+key] = test_out[:, control_horizon + num_states*(i-1)+key] + diff

    # apply control rate bound on train
    for i in range(0):
        if i == 0:
            diff = train_out[:, i] - train_obs[:, -1]
        else:
            diff = train_out[:, i] - train_out[:, i-1]
        diff = np.clip(
            diff,
            -control_bound+gap_ctrl,
            control_bound-gap_ctrl,
        )
        if i == 0:
            train_out[:, i] = diff + train_obs[:, -1]
        else:
            train_out[:, i] = diff + train_out[:, i-1]

    # apply control rate bound on test
    for i in range(0):
        if i == 0:
            diff = test_out[:, i] - test_obs[:, -1]
        else:
            diff = test_out[:, i] - test_out[:, i-1]
        diff = np.clip(
            diff,
            -control_bound+gap_ctrl,
            control_bound-gap_ctrl,
        ...
        if i == 0:
            test_out[:, i] = diff + test_obs[:, -1]
        else:
            test_out[:, i] = diff + test_out[:, i-1]

    return train_out, test_out


def generateDataWindowForce6(window_size, ctrl_horizon):
    # obs = [femur angle, vel, tibia angle, vel] * window_size
    # ctrl = [ankle angle] * ctrl_horizon
    # output = [femur angle, vel, tibia angle, vel, force] * ctrl_horizon
    pad = window_size + ctrl_horizon
    ...
    # create training dataset
    obs_train = np.array([]).reshape(0, 9 * window_size)
    for i in range(walking_ranges[0][0],walking_ranges[0][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
    ...
    # create validation dataset
    obs_val = np.array([]).reshape(0, 9 * window_size)
    for i in range(walking_ranges[1][0], walking_ranges[1][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
    ...
    # create test dataset
    obs_test = np.array([]).reshape(0, 9 * window_size)
    for i in range(walking_ranges[2][0], walking_ranges[2][1]):
        temp_obs = np.array([]).reshape(1, 0)
        for j in range(window_size):
            temp_obs = np.concatenate(
...
if __name__ == "__main__":
    ...
    orig_load = True
    # trained_time_str = "_2_23_2023_21_31_41"
    trained_time_str = "_2_25_2023_10_47_36"  # trained model
    # trained_time_str = "_2_24_2023_1_39_41"
    # Train window model
    # (
    #     train_obs,
    #     train_out,
    ...
    (
        ...
        a_p_test,
        s_p_test,
    ) = generateDataWindowForce6(10, control_horizon)
    train_obs = s_h_train
    ...
    if orig_load:
        ...
        ctrl_model_orig.fit(
            ...
            validation_data=(
                val_obs, [val_out[:, control_horizon:], val_out[:, 0:control_horizon]]
                ),
    ...
    ctrl_model_orig.layers[7].trainable = False
    # create new model 
    ctrl_model_orig2, _, _ = buildModelWindow(
            train_obs.shape, ctrl_layer_size, pred_layer_size, 0.001
        )
    # copy weights
    for i in range(6, 8):
        ctrl_model_orig2.layers[i].set_weights(ctrl_model_orig.layers[i].get_weights())
    # freeze weights
    for i in range(len(ctrl_model_orig.layers)):
        ctrl_model_orig2.layers[i].trainable = False
    # unfreeze weights
    
    ctrl_model_orig2.layers[1].trainable = True
    ctrl_model_orig2.layers[2].trainable = True
    ctrl_model_orig2.layers[3].trainable = True
    ctrl_model_orig2.layers[6].trainable = False
    ctrl_model_orig2.layers[7].trainable = False
    ctrl_model_orig = ctrl_model_orig2
    ...
    def loss2(y_true, y_pred):
        loss = 1*tf.keras.losses.MSE(y_true, y_pred)
        return loss
    ...
    def loss_state(y_true, y_pred):
        loss = 0
        for i in range(control_horizon):
            # w = control_horizon - control_horizon/(1+np.exp(-(i-7)))
            w = 10-(9/(control_horizon-1))*i
            # print(w)
            for j in range(num_states_obs+num_states_non_obs):
                loss += w * tf.keras.losses.MSE(
                    y_true[:, i*(num_states_obs+num_states_non_obs)+j],
                    y_pred[:, i*(num_states_obs+num_states_non_obs)+j],
                    )
        return loss

    def loss_state2(y_true, y_pred):
        loss = 0
        for i in range(control_horizon):
            if i == 0:
                w = 10
    ...
    def loss_ctrl2(y_true, y_pred):
        ...
        for i in range(control_horizon):
            ...
            # print(w)
        return loss
    #specify weights 1 to 1 and 2 loss terms 
    ctrl_model_orig.compile(
        optimizer=keras.optimizers.Adam(learning_rate=0.001),
        loss=[loss3, loss2],
        # metrics=["accuracy"],
    ...
    train_out, val_out = hand_labeling_data(
        train_obs,
        train_out,
        val_obs,
        val_out,
        control_horizon,
        control_bound=5.,
        state_bounds={4: [-.35, .35], 5: [-.35, .35], 6: [-.35, .35], 7: [-.35, .35]},
        # state_bounds={},
        gap_state=.2,
        gap_ctrl=3,
        num_states=num_states_obs + num_states_non_obs,
        )
    # train_out, val_out = hand_labeling_data2(
    #     train_out,
    #     val_out,
    #     control_horizon,
    #     {4: [-1.4, .9], 5: [-1.4, .9], 6: [-1.4, .9], 7: [-1.4, .9]},
    #     0.8,
