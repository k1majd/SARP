...
def generateDataWindowForcenCrtl(window_size, ctrl_horizon):
...
def buildModelWindow(
    ...
    tf_callback = [
        ...
        keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", factor=0.1, patience=50, min_lr=0.0001,
        ),
...
def hand_labeling_data(train_obs, train_out, test_obs, test_out, control_horizon, control_bound, state_bounds, gap_state=0.2, gap_ctrl=1., num_states=6):
    for key, value in state_bounds.items():
        for i in range(control_horizon):
            train_out[:, control_horizon + num_states*i+key] = np.clip(
                train_out[:, control_horizon + num_states*i+key],
                value[0]+gap_state, value[1]-gap_state
                )
            test_out[:, control_horizon + num_states*i+key] = np.clip(
                test_out[:, control_horizon + num_states*i+key],
                value[0]+gap_state, value[1]-gap_state
...
if __name__ == "__main__":
    ...
    (
        ...
        a_p_test,
        s_p_test,
    ) = generateDataWindowForcenCrtl(10, control_horizon)
    train_obs = s_h_train
    ...
    train_out, val_out = hand_labeling_data(
        ...
        control_horizon,
        control_bound=4.,
        # state_bounds={4: [-1.4, .9], 5: [-1.4, .9], 6: [-1.4, .9], 7: [-1.4, .9]},
        state_bounds={},
        gap_state=.5,
        gap_ctrl=3,
